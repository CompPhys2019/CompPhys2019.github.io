C subroutine diag(nd,vec,val)
C     implicit none
C     integer::nd,lwork,info
C     real(8)::vec(nd,nd),val(nd),work(3*nd-1)

C     lwork = 3*nd-1

C     call Ssyev('V','U',nd,vec,nd,val,work,lwork,info)
    
C end subroutine diag
...
*          where NB is the blocksize for SSYTRD returned by SILAENV.
*
*          If LWORK = -1, then a workspace query is assumed; the routine
*          only calculates the optimal size of the WORK array, returns
*          this value as the first entry of the WORK array, and no error
*          message related to LWORK is issued by SXERBLA.
*
*  INFO    (output) INTEGER
*          = 0:  successful exit
...
*     .. External Functions ..
      ...
      INTEGER            SILAENV
      REAL   SLAMCH, SLANSY
      EXTERNAL           SLSAME, SILAENV, SLAMCH, SLANSY
*     ..
...
*
      INFO = 0
      IF( .NOT.( WANTZ .OR. SLSAME( JOBZ, 'N' ) ) ) THEN
         INFO = -1
      ELSE IF( .NOT.( LOWER .OR. SLSAME( UPLO, 'U' ) ) ) THEN
         INFO = -2
      ELSE IF( N.LT.0 ) THEN
         INFO = -3
...
*
      IF( INFO.EQ.0 ) THEN
         NB = SILAENV( 1, 'SSYTRD', UPLO, N, -1, -1, -1 )
         LWKOPT = MAX( 1, ( NB+2 )*N )
         WORK( 1 ) = LWKOPT
      END IF
*
      IF( INFO.NE.0 ) THEN
         CALL SXERBLA( 'SSYEV ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SAXPY, SLARFG, SSYMV, SSYR2, SXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            SLSAME
      REAL   SDOT
      EXTERNAL           SLSAME, SDOT
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      INFO = 0
      UPPER = SLSAME( UPLO, 'U' )
      IF( .NOT.UPPER .AND. .NOT.SLSAME( UPLO, 'L' ) ) THEN
         INFO = -1
      ELSE IF( N.LT.0 ) THEN
         INFO = -2
      ELSE IF( LDA.LT.MAX( 1, N ) ) THEN
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SLATRD, SSYR2K, SSYTD2, SXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*
         NX = MAX( NB, SILAENV( 3, 'SSYTRD', UPLO, N, -1, -1, -1 ) )
         IF( NX.LT.N ) THEN
*
...
*
               ...
               NBMIN = SILAENV( 2, 'SSYTRD', UPLO, N, -1, -1, -1 )
               IF( NB.LT.NBMIN )
     $            NX = N
            END IF
         ELSE
...
*
      END
      INTEGER          FUNCTION SIEEECK( ISPEC, ZERO, ONE )
*
...
*  =======
*
*  SIEEECK is called from the SILAENV to verify that Infinity and
...
*
      RETURN
      END
      INTEGER          FUNCTION SILAENV( ISPEC, NAME, OPTS, N1, N2, N3,
     $                 N4 )
*
*  -- LAPACK auxiliary routine (version 3.0) --
*     Univ. of Tennessee, Univ. of California Berkeley, NAG Ltd.,
...
*  =======
*
*  SILAENV is called from the LAPACK routines to choose problem-dependent
...
*               where k is given by SILAENV(2,...) and m by SILAENV(5,...)
...
*          be required.
*
* (SILAENV) (output) INTEGER
*          >= 0: the value of the parameter specified by ISPEC
*          < 0:  if SILAENV = -k, the k-th argument had an illegal value.
*
*  Further Details
*  ===============
*
*  The following conventions have been used when calling SILAENV from the
*  LAPACK routines:
...
*      passed a value of -1.
*  3)  The parameter value returned by SILAENV is checked for validity in
*      the calling subroutine.  For example, SILAENV is used to retrieve
*      the optimal blocksize for STRTRI as follows:
*
*      NB = SILAENV( 1, 'STRTRI', UPLO // DIAG, N, -1, -1, -1 )
...
*
      SILAENV = INT( REAL( MIN( N1, N2 ),16 )*1.6E0 )
      RETURN
*
  700 CONTINUE
*
...
C     SILAENV = 0
      SILAENV = 1
      IF( SILAENV.EQ.1 ) THEN
         SILAENV = SIEEECK( 0, 0.0, 1.0 ) 
      END IF
      RETURN
*
 1100 CONTINUE
...
*  =======
*
*  SLSAME returns .TRUE. if CA is the same letter as CB regardless of
*  case.
*
*  Arguments
*  =========
...
*
      END
      SUBROUTINE SXERBLA( SRNAME, INFO )
*
*  -- LAPACK auxiliary routine (version 3.0) --
*     Univ. of Tennessee, Univ. of California Berkeley, NAG Ltd.,
*     Courant Institute, Argonne National Lab, and Rice University
...
*  =======
*
*  SXERBLA  is an error handler for the LAPACK routines.
...
*          The name of the routine which called SXERBLA.
*
*  INFO    (input) INTEGER
...
*  =======
*
*  SLAMCH determines REAL machine parameters.
*
*  Arguments
*  =========
*
...
*
      ...
      END IF
*
      IF( SLSAME( CMACH, 'E' ) ) THEN
         RMACH = EPS
      ELSE IF( SLSAME( CMACH, 'S' ) ) THEN
         RMACH = SFMIN
      ELSE IF( SLSAME( CMACH, 'B' ) ) THEN
         RMACH = BASE
      ELSE IF( SLSAME( CMACH, 'P' ) ) THEN
         RMACH = PREC
      ELSE IF( SLSAME( CMACH, 'N' ) ) THEN
         RMACH = T
      ELSE IF( SLSAME( CMACH, 'R' ) ) THEN
         RMACH = RND
      ELSE IF( SLSAME( CMACH, 'M' ) ) THEN
         RMACH = EMIN
      ELSE IF( SLSAME( CMACH, 'U' ) ) THEN
         RMACH = RMIN
      ELSE IF( SLSAME( CMACH, 'L' ) ) THEN
         RMACH = EMAX
      ELSE IF( SLSAME( CMACH, 'O' ) ) THEN
         RMACH = RMAX
      END IF
*
      SLAMCH = RMACH
...
*
   ...
   10    CONTINUE
      ELSE IF( SLSAME( NORM, 'O' ) .OR. NORM.EQ.'1' .OR.
     $         SLSAME( NORM, 'I' ) ) THEN
*
*        Find norm1(A).
*
   ...
   20       CONTINUE
         END IF
      ELSE IF( ( SLSAME( NORM, 'F' ) ) .OR. ( SLSAME( NORM, 'E' ))) THEN
*
*        Find normF(A).
*
         SCALE = ZERO
...
*
   ...
   40       CONTINUE
         END IF
      ELSE IF( ( SLSAME( NORM, 'I' ) ) .OR. ( SLSAME( NORM, 'O' ) ) .OR.
     $         ( NORM.EQ.'1' ) ) THEN
*
...
*
     ...
     $   RETURN
*
      IF( SLSAME( TRANS, 'N' ) ) THEN
         TRANST = 'T'
      ELSE
         TRANST = 'N'
      END IF
*
      IF( SLSAME( STOREV, 'C' ) ) THEN
*
         IF( SLSAME( DIRECT, 'F' ) ) THEN
*
...
*
      INFO = 0
*
      IF( SLSAME( TYPE, 'G' ) ) THEN
         ITYPE = 0
      ELSE IF( SLSAME( TYPE, 'L' ) ) THEN
         ITYPE = 1
      ELSE IF( SLSAME( TYPE, 'U' ) ) THEN
         ITYPE = 2
      ELSE IF( SLSAME( TYPE, 'H' ) ) THEN
         ITYPE = 3
      ELSE IF( SLSAME( TYPE, 'B' ) ) THEN
         ITYPE = 4
      ELSE IF( SLSAME( TYPE, 'Q' ) ) THEN
         ITYPE = 5
      ELSE IF( SLSAME( TYPE, 'Z' ) ) THEN
         ITYPE = 6
      ELSE
         ITYPE = -1
      END IF
...
*
      INFO = 0
      IF( .NOT.( SLSAME( SIDE, 'L' ) .OR. SLSAME( SIDE, 'R' ) ) ) THEN
         INFO = 1
      ELSE IF( .NOT.( SLSAME( PIVOT, 'V' ) .OR. SLSAME( PIVOT,
     $         'T' ) .OR. SLSAME( PIVOT, 'B' ) ) ) THEN
         INFO = 2
      ELSE IF( .NOT.( SLSAME( DIRECT, 'F' ) .OR. SLSAME( DIRECT, 'B' )))
     $          THEN
         INFO = 3
      ELSE IF( M.LT.0 ) THEN
         INFO = 4
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SLARFB, SLARFT, SORG2L, SXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*        Determine when to cross over from blocked to unblocked code.
*
         NX = MAX( 0, SILAENV( 3, 'SORGQL', ' ', M, N, K, -1 ) )
         IF( NX.LT.K ) THEN
*
*           Determine if workspace is large enough for blocked code.
*
...
*
               NB = LWORK / LDWORK
               NBMIN = MAX( 2, SILAENV( 2, 'SORGQL', ' ', M, N, K, -1 ))
            END IF
         END IF
      END IF
*
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SLARFB, SLARFT, SORG2R, SXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*        Determine when to cross over from blocked to unblocked code.
*
         NX = MAX( 0, SILAENV( 3, 'SORGQR', ' ', M, N, K, -1 ) )
         IF( NX.LT.K ) THEN
*
*           Determine if workspace is large enough for blocked code.
*
...
*
               NB = LWORK / LDWORK
               NBMIN = MAX( 2, SILAENV( 2, 'SORGQR', ' ', M, N, K, -1 ))
            END IF
         END IF
      END IF
*
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SORGQL, SORGQR, SXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*
      IF( INFO.EQ.0 ) THEN
         IF( UPPER ) THEN
            NB = SILAENV( 1, 'SORGQL', ' ', N-1, N-1, N-1, -1 )
         ELSE
            NB = SILAENV( 1, 'SORGQR', ' ', N-1, N-1, N-1, -1 )
         END IF
         LWKOPT = MAX( 1, N-1 )*NB
         WORK( 1 ) = LWKOPT
      END IF
...
*  if SSYTRD or DSPTRD or DSBTRD has been used to reduce this matrix to
*  tridiagonal form.
*
*  Arguments
*  =========
...
*     .. Parameters ..
      ...
      PARAMETER          ( ZERO = 0.00, ONE = 1.00, TWO = 2.00,
     $                   THREE = 3.00 )
      INTEGER            MAXIT
      PARAMETER          ( MAXIT = 30 )
*     ..
...
*     .. External Functions ..
      ...
      EXTERNAL           SLSAME, SLAMCH, SLANST, SLAPY2
*     ..
*     .. External Subroutines ..
      EXTERNAL           SLAE2, SLAEV2, SLARTG, SLASCL, SLASET, SLASR,
     $                   SLASRT, SSWAP, SXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      INFO = 0
*
      IF( SLSAME( COMPZ, 'N' ) ) THEN
         ICOMPZ = 0
      ELSE IF( SLSAME( COMPZ, 'V' ) ) THEN
         ICOMPZ = 1
      ELSE IF( SLSAME( COMPZ, 'I' ) ) THEN
         ICOMPZ = 2
      ELSE
         ICOMPZ = -1
      END IF
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           SLAE2, SLASCL, SLASRT, SXERBLA
*     ..
*     .. Intrinsic Functions ..
...
* BLAS REQUIRED BY LAPACK ROUTINE:  Ssyev
* -----------------------------------------------------------
...
*
      INFO = 0
      IF     ( .NOT.SLSAME( TRANS, 'N' ).AND.
     $         .NOT.SLSAME( TRANS, 'T' ).AND.
     $         .NOT.SLSAME( TRANS, 'C' )      )THEN
         INFO = 1
      ELSE IF( M.LT.0 )THEN
         INFO = 2
      ELSE IF( N.LT.0 )THEN
...
*
      INFO = 0
      IF     ( .NOT.SLSAME( UPLO, 'U' ).AND.
     $         .NOT.SLSAME( UPLO, 'L' )      )THEN
         INFO = 1
      ELSE IF( N.LT.0 )THEN
         INFO = 2
      ELSE IF( LDA.LT.MAX( 1, N ) )THEN
...
*
     ...
     $         .NOT.SLSAME( UPLO , 'L' )      )THEN
         INFO = 1
      ELSE IF( .NOT.SLSAME( TRANS, 'N' ).AND.
     $         .NOT.SLSAME( TRANS, 'T' ).AND.
     $         .NOT.SLSAME( TRANS, 'C' )      )THEN
         INFO = 2
      ELSE IF( .NOT.SLSAME( DIAG , 'U' ).AND.
     $         .NOT.SLSAME( DIAG , 'N' )      )THEN
         INFO = 3
      ELSE IF( N.LT.0 )THEN
         INFO = 4
      ELSE IF( LDA.LT.MAX( 1, N ) )THEN
...
*
      NOTA  = SLSAME( TRANSA, 'N' )
      NOTB  = SLSAME( TRANSB, 'N' )
      IF( NOTA )THEN
         NROWA = M
         NCOLA = K
      ELSE
...
*
      INFO = 0
      IF(      ( .NOT.NOTA                 ).AND.
     $         ( .NOT.SLSAME( TRANSA, 'C' ) ).AND.
     $         ( .NOT.SLSAME( TRANSA, 'T' ) )      )THEN
         INFO = 1
      ELSE IF( ( .NOT.NOTB                 ).AND.
     $         ( .NOT.SLSAME( TRANSB, 'C' ) ).AND.
     $         ( .NOT.SLSAME( TRANSB, 'T' ) )      )THEN
         INFO = 2
      ELSE IF( M  .LT.0               )THEN
         INFO = 3
      ELSE IF( N  .LT.0               )THEN
...
*
      INFO   = 0
      IF(      ( .NOT.LSIDE                ).AND.
     $         ( .NOT.SLSAME( SIDE  , 'R' ) )      )THEN
         INFO = 1
      ELSE IF( ( .NOT.UPPER                ).AND.
     $         ( .NOT.SLSAME( UPLO  , 'L' ) )      )THEN
         INFO = 2
      ELSE IF( ( .NOT.SLSAME( TRANSA, 'N' ) ).AND.
     $         ( .NOT.SLSAME( TRANSA, 'T' ) ).AND.
     $         ( .NOT.SLSAME( TRANSA, 'C' ) )      )THEN
         INFO = 3
      ELSE IF( ( .NOT.SLSAME( DIAG  , 'U' ) ).AND.
     $         ( .NOT.SLSAME( DIAG  , 'N' ) )      )THEN
         INFO = 4
